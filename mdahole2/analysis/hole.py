...
def hole(pdbfile,
    ...
    Only a subset of all `HOLE control parameters
    <http://www.holeprogram.org/doc/old/hole_d03.html>`_ is supported and can
    be set with keyword arguments.
    ...
    output_level : int, optional
        ...
        turned off.
    dcd : str, optional
        File name of CHARMM-style DCD trajectory (must be supplied together
        with a matching PDB file `filename`) and then HOLE runs its analysis on
        ...
        determined for each individual structure are written in a tagged format
        so that it is possible to extract the information from the text output
        file using a :program:`grep` command. The reading of the file can be
        controlled by the ``dcd_step`` keyword and/or setting ``dcd_iniskip``
        to the number of frames to be skipped initially.
...
class HoleAnalysis(AnalysisBase):
    r"""Run :program:`hole` on a trajectory.
    ...
    def __init__(self, universe,
        ...
        super(HoleAnalysis, self).__init__(universe.universe.trajectory,
                                           verbose=verbose)
        if output_level > 3:
            msg = ('output_level ({}) needs to be < 3 in order to extract a'
                   ' HOLE profile!')
            warnings.warn(msg.format(output_level))
        ...
        if isinstance(cpoint, str):
            if 'geometry' in cpoint.lower():
                ...
                self.cpoint = ('{cpoint[0]:.10f} {cpoint[1]:.10f}'
                               ' {cpoint[2]:.10f}')
        else:
    ...
    def create_vmd_surface(self, filename='hole.vmd', dot_density=15,
        ...
        The level of detail is determined by ``dot_density``. The surface
        will be colored by ``no_water_color``, ``one_water_color``, and
    ...
    def bin_radii(self, frames=None, bins=100, range=None):
        ...
        range : (float, float), optional
            ...
            is simply ``(a.min(), a.max())``, where ``a`` is the array of
            reaction coordinates. Values outside the range are ignored. The
            first element of the range must be less than or equal to the
            second.
    ...
    def histogram_radii(self, aggregator=np.mean, frames=None,
        ...
        range : (float, float), optional
            ...
            is simply ``(a.min(), a.max())``, where ``a`` is the array of
            reaction coordinates. Values outside the range are ignored. The
            first element of the range must be less than or equal to the second
